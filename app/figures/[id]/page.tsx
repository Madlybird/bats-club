...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    ...
    console.error("[figures/[id]] generateMetadata error:", e)
...
export default async function FigureDetailPage({ params }: Props) {
  ...
  // Main figure fetch — use maybeSingle so a missing row returns
  // { data: null } instead of an error, and wrap in try/catch so any
  // unexpected Supabase failure surfaces as a 404 rather than a 500.
  let figure: any = null
  try {
    const { data, error } = await supabaseAdmin
      ...
      .maybeSingle()
    if (error) {
      console.error("[figures/[id]] main query error:", error)
      notFound()
    }
    figure = data
  } catch (e: any) {
    ...
    console.error("[figures/[id]] main query threw:", e)
    notFound()
  }
  ...
  // Related figures are best-effort — if either query fails we just
  // render an empty related-section instead of crashing the whole page.
  ...
  try {
    ...
    console.error("[figures/[id]] related figures query failed:", e)
    relatedFigures = []
  }
