...
export default async function ProfilePageRu(props: Props) {
  ...
  return (
    <ProfilePageContent user={user as any} have={have} wishlist={wishlist} dict={ru} archiveHref="/ru/archive" profileBasePath="/ru/profile"
      isOwner={session?.user?.id === userId} purchaseCount={purchaseCount}
      huntingCounts={huntingCounts} activeListings={activeListings as any} locale="ru" />
  )
}
