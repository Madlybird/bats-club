...
const getFigureCore = cache(async (figureId: string) => {
  const { data, error } = await supabaseAdmin
    .from("figures")
    .select(
      "id, slug, name, series, character, manufacturer, scale, year, sculptor, material, imageUrl:image_url, images, description, description_ru, createdAt:created_at"
    )
    .eq("id", figureId)
    .maybeSingle()
  if (error) {
    // description_ru column may be missing in older deploys; retry without it.
...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  try {
    ...
    return {
      ...
      ...figurePageMetadata(slugForUrl, "ru", title, image, activeListingId),
    }
  } catch (e: any) {
...
export default async function FigureDetailPageRu(props: Props) {
  ...
  try {
    ...
    if (e?.digest?.startsWith?.("NEXT_")) throw e
    console.error("[ru/figures/[slug]] figure query threw:", e)
    notFound()
  }
  if (!figure) notFound()
  ...
  const [listingsRes, userFiguresRes, articleFiguresRes, rates] = await Promise.all([
    ...
    supabaseAdmin
      ...
      .eq("figure_id", figureId),
    getRates().catch((e) => {
      console.error("[ru/figures/[slug]] rates failed:", e)
      return null
    }),
  ])
  ...
  const convertedLowestPrice = rates && lowestPrice !== null ? convertPrice(lowestPrice, "ru", rates) : null
  ...
  return (
    <FigureDetailContent
      figure={{ ...figure, descriptionLocale: (figure as any).description_ru || null } as any}
