...
export default function ShareButtons({ name, label }: Props) {
  ...
  const btnClass =
    "flex items-center justify-center w-8 h-8 rounded-lg border border-white/[0.08] bg-white/[0.03] text-white/40 hover:text-white hover:border-white/20 hover:bg-white/[0.06] transition-all"

  return (
    <div className="flex items-center gap-2 mt-3">
      <span className="text-xs text-white/25 font-medium uppercase tracking-wider mr-1">
        {label}
      </span>

      {/* Telegram */}
      <a
        href={`https://t.me/share/url?url=${encodedUrl}&text=${encodedName}`}
        target="_blank"
      ...
      <button
        onClick={handleCopy}
        title="Copy link"
        className={`${btnClass} ${copied ? "border-emerald-500/40 text-emerald-400" : ""}`}
      >
        {copied ? (
